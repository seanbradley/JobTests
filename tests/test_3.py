...
Examples:
Pattern: "ABC*EF*****6" matches against "ABCAEF111116", but not "ABC1EFAAAAA6"  USE FORM VALIDATION / OVERRIDE CLEAN METHOD    ^[a-zA-Z]{6}[0-9]{6}$
If there are two patterns: "ABCDEF******" and "ABCDEF1*****", the string "ABCDEF123456" would match against the second pattern because it has fewer asterisks.  FOR ALL RESULTS: COUNT ASTERICKS; FEWER ASTERICKS WINS
